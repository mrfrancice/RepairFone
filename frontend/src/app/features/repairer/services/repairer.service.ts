...
export type RequestFilterStatus = 'new' | 'accepted' | 'in_progress' | 'completed' | 'all';
...
export class RepairerService {
  ...
  // Specialties list
  readonly availableSpecialties = [
    { id: 'screen', label: 'Écran / Vitre', icon: '📱' },
    { id: 'battery', label: 'Batterie', icon: '🔋' },
    { id: 'charging', label: 'Port de charge', icon: '🔌' },
    { id: 'camera', label: 'Caméra', icon: '📷' },
    { id: 'speaker', label: 'Haut-parleur / Micro', icon: '🔊' },
    { id: 'button', label: 'Boutons', icon: '⚙️' },
    { id: 'water', label: 'Dégâts des eaux', icon: '💧' },
    { id: 'software', label: 'Logiciel / OS', icon: '💻' },
    { id: 'data', label: 'Récupération données', icon: '💾' },
    { id: 'unlock', label: 'Déblocage', icon: '🔓' },
  ];

  // Get current repairer profile
  async getMyProfile(): Promise<RepairerProfile> {
    return firstValueFrom(
      this.api.get<RepairerProfile>('/repairer/profile')
    );
  }
  ...
  async updateProfile(dto: UpdateProfileDto): Promise<RepairerProfile> {
    ...
    try {
      const result = await firstValueFrom(
        this.api.patch<RepairerProfile>('/repairer/profile', dto)
      );
      return result;
    } catch (err: any) {
