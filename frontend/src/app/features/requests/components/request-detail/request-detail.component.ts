  ...
  template: `
    <div class="detail-container">
      @if (isLoading()) {
        ...
        <ui-header
          [title]="headerTitle()"
          [subtitle]="request()?.serviceType?.name || 'Demande de réparation'"
          [showBack]="true"
          [showProfile]="true"
          backRoute="/requests"
        >
        ...
        <div class="detail-content">
          ...
          <!-- Section Noter le réparateur (Client uniquement) -->
          @if (isClient() && canShowRatingSection()) {
            <section class="section rating-section">
              <h2>
                <div class="section-icon-wrapper icon-yellow">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              ...
              @if (!showRatingForm()) {
                <div class="rating-prompt">
                  <p class="rating-intro">Comment se passe votre expérience avec ce réparateur ?</p>
                  <div class="rating-steps-grid">
                    @for (step of getAvailableRatingSteps(); track step.key) {
                      <button
                        class="rating-step-btn"
                        [class.rated]="isStepRated(step.key)"
                        [disabled]="isStepRated(step.key)"
                        (click)="openRatingForm(step.key)"
                      >
                        <span class="step-icon">{{ getStepIcon(step.key) }}</span>
                        <span class="step-label">{{ step.label }}</span>
                        @if (isStepRated(step.key)) {
                  ...
                  @if (stepRatings().length > 0) {
                    <div class="existing-ratings-card">
                      <h4>Vos notes</h4>
                      <div class="ratings-list">
                        @for (rating of stepRatings(); track rating.id) {
                          <div class="rating-row">
                            <span class="rating-cat">{{ reviewsService.getCategoryLabel(rating.category) }}</span>
                            <span
                              class="rating-val"
                              [style.color]="reviewsService.getStepRatingColor(rating.rating)"
                            >
                              {{ reviewsService.formatStepRating(rating.rating) }}
                            </span>
                          </div>
                        }
                      </div>
...
export class RequestDetailComponent implements OnInit {
  ...
  readonly currentRatingStep = signal<RatingStep | null>(null);
  readonly stepRatings = signal<StepRating[]>([]);
  readonly ratedSteps = signal<Set<string>>(new Set());

  // Computed for header title
  readonly headerTitle = computed(() => {
    const req = this.request();
    if (!req?.device) return 'Détail de la demande';
    const brand = req.device.brand || '';
    const model = req.device.model || '';
    const title = `${brand} ${model}`.trim();
    return title || 'Détail de la demande';
  });
  ...
  canShowRatingSection(): boolean {
    const req = this.request();
    if (!req) return false;
    // Show rating only for accepted, completed, or delivered requests
    // Rating starts when the repairer has accepted and a quote is involved
    return ['accepted', 'completed', 'delivered'].includes(req.status);
  }

  getAvailableRatingSteps(): { key: RatingStep; label: string }[] {
    const req = this.request();
    if (!req) return [];

    const steps: { key: RatingStep; label: string }[] = [];
    const status = req.status;

    // Rating available from "accepted" status onwards
    if (['accepted', 'completed', 'delivered'].includes(status)) {
      steps.push({ key: RatingStep.QUOTE_ACCEPTED, label: 'Devis accepté' });
      steps.push({ key: RatingStep.IN_PROGRESS, label: 'En cours' });
    }
    if (['completed', 'delivered'].includes(status)) {
      steps.push({ key: RatingStep.COMPLETED, label: 'Réparation terminée' });
    }
    if (status === 'delivered') {
      steps.push({ key: RatingStep.DELIVERED, label: 'Livraison' });
    }

    return steps;
  }

  getStepIcon(step: RatingStep): string {
    switch (step) {
  ...
  isStepRated(step: RatingStep): boolean {
    return this.ratedSteps().has(step);
  }

  openRatingForm(step: RatingStep): void {
    this.currentRatingStep.set(step);
    this.showRatingForm.set(true);
  }

  closeRatingForm(): void {
    this.showRatingForm.set(false);
    this.currentRatingStep.set(null);
  }

  async onRatingSubmitted(): Promise<void> {
    const step = this.currentRatingStep();
    if (step) {
      this.ratedSteps.update(set => {
  ...
  private async loadStepRatings(): Promise<void> {
    const req = this.request();
    if (!req) return;

    try {
      const result = await this.reviewsService.getStepRatingsForRequest(req.id);
      this.stepRatings.set(result.ratings);

      const steps = new Set<string>();
      for (const rating of result.ratings) {
        steps.add(rating.step);
      }
      this.ratedSteps.set(steps);
    } catch {
      // Silently fail
    }
  }
