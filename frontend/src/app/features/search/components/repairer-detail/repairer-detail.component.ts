...
@Component({
              ...
              @for (score of qualityScores(); track score.label) {
                <div class="quality-item">
                  ...
                  <div class="quality-bar">
                    <div class="quality-fill" [style.width.%]="score.value"></div>
                  </div>
                  <span class="quality-value">{{ score.value }}%</span>
                </div>
              }
            </div>
          </section>
  ...
  styles: [`
    ...
    .quality-fill {
      height: 100%;
      background: linear-gradient(90deg, #FF6B35, #F9A825, #4CAF50);
      border-radius: 5px;
...
export class RepairerDetailComponent implements OnInit {
  ...
  readonly qualityScores = signal<QualityScore[]>([
    { label: 'Délais', value: 92, icon: '⏱️' },
    { label: 'Qualité', value: 95, icon: '✨' },
    { label: 'Communication', value: 88, icon: '💬' },
    { label: 'Prix', value: 90, icon: '💰' },
  ]);
