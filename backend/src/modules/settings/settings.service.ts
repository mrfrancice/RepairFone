...
export class SettingsService implements OnModuleInit {
  ...
  private async seedSpecialties() {
    ...
    const specialties = [
      // Marques
      {
        ...
        name: 'iPhone',
        icon: '📱',
        category: SpecialtyCategory.BRAND,
        sortOrder: 1,
      },
      {
        code: 'samsung',
      ...
      // Types d'appareils
      {
        ...
        name: 'Tablettes',
        icon: '📲',
        category: SpecialtyCategory.DEVICE_TYPE,
        sortOrder: 10,
      },
      {
        code: 'ipad',
      ...
      // Types de réparation
      {
        ...
        name: 'Écrans',
        icon: '📱',
        category: SpecialtyCategory.REPAIR_TYPE,
        sortOrder: 20,
      },
      {
        code: 'battery',
    ...
    await this.specialtyRepo.save(
      specialties.map((s) => this.specialtyRepo.create(s)),
    );
  }
  ...
  private async seedProblemCategories() {
    ...
    await this.problemCategoryRepo.save(
      categories.map((c) => this.problemCategoryRepo.create(c)),
    );
  }
  ...
  private async seedBusinessTypes() {
    ...
    const types = [
      {
        code: 'individual',
        name: 'Particulier / Artisan',
        description: 'Réparateur indépendant sans structure juridique',
        requiresRccm: false,
        requiresTaxId: false,
        sortOrder: 1,
      },
      {
        ...
        description: 'Micro-entreprise ou auto-entrepreneur',
        requiresRccm: true,
        requiresTaxId: true,
        sortOrder: 2,
      },
      {
        code: 'company',
        name: 'Entreprise (SARL, SA...)',
        description: 'Société avec forme juridique',
        requiresRccm: true,
        requiresTaxId: true,
        sortOrder: 3,
      },
    ...
    await this.businessTypeRepo.save(
      types.map((t) => this.businessTypeRepo.create(t)),
    );
  }
  ...
  private async seedExperienceRanges() {
    ...
    await this.experienceRangeRepo.save(
      ranges.map((r) => this.experienceRangeRepo.create(r)),
    );
  }
    ...
    const types = [
      {
        ...
        description: 'CNI ivoirienne en cours de validité',
        sortOrder: 1,
      },
      {
        code: 'passport',
      ...
      },
      {
        ...
        description: 'Carte de séjour pour les étrangers',
        sortOrder: 4,
      },
    ];

    await this.idDocumentTypeRepo.save(
      types.map((t) => this.idDocumentTypeRepo.create(t)),
    );
  }
  ...
  private async seedBadgeTypes() {
    ...
    const badges = [
      ...
      },
      {
        ...
        icon: '🏆',
        color: '#8b5cf6',
        description: 'Plus de 100 réparations réussies',
        sortOrder: 4,
      },
    ];

    await this.badgeTypeRepo.save(
      badges.map((b) => this.badgeTypeRepo.create(b)),
    );
  }
  ...
  private async seedAppConfigs() {
    ...
    const configs = [
      ...
      },
      {
        ...
        value: '30',
        type: 'number',
        description: "Pourcentage d'acompte requis",
        category: 'payment',
      },
      {
        ...
        value: '7',
        type: 'number',
        description: 'Durée de validité des devis (jours)',
        category: 'quotes',
      },
      {
        key: 'max_home_service_radius_km',
        value: '50',
        type: 'number',
        description: 'Rayon maximum pour service à domicile (km)',
        category: 'service',
      },
      {
        key: 'currency',
    ...
    await this.appConfigRepo.save(
      configs.map((c) => this.appConfigRepo.create(c)),
    );
  }
  ...
  async getProblemCategories(activeOnly = true): Promise<ProblemCategory[]> {
    const where = activeOnly ? { isActive: true } : {};
    return this.problemCategoryRepo.find({
      where,
      order: { sortOrder: 'ASC' },
    });
  }
  ...
  async getExperienceRanges(activeOnly = true): Promise<ExperienceRange[]> {
    const where = activeOnly ? { isActive: true } : {};
    return this.experienceRangeRepo.find({
      where,
      order: { sortOrder: 'ASC' },
    });
  }
